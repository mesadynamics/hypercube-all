...
$template = sprintf("user_id=%s&session_key=%s", $user, $session);
...
$query = sprintf("select friendster_id, cube_id, widget_id from friendster where id = '%s'", $user); 
$result = query($query);
if ($row = mysql_fetch_assoc($result)) {
	...
	$query = sprintf("insert into friendster (id, cube_id) values ('%s', %s)", $user, $cubeid);
	query($query);
