...
if (isset($_REQUEST['create'])) {
	...
	if (isset($code) && isset($title) && $code != '' && $title != '') {
		...
		if (mysql_fetch_assoc($result)) {
			$url = sprintf("errormsg.php?%s&errtitle=Erro ao criar Widget&errmsg=0",
				$template);
			redirect($url);
			exit;
		}
		...
		$url = sprintf("errormsg.php?%s&errtitle=Erro ao criar Widget&errmsg=1",
			$template);
		redirect($url);
		exit;
	}
	...
	if ($row = mysql_fetch_assoc($result)) {
		...
		if($cube_id == $currentcube) {
			...
			if(!isset($title) || $title == '') {
				$url = sprintf("error.php?%s&errtitle=Erro ao copiar Widget&errmsg=Digite um nome no campo Nome.",
					$template);
				redirect($url);	
				exit;
			}
			...
			if (mysql_fetch_assoc($result)) {
				$url = sprintf("errormsg.php?%s&errtitle=Erro ao copiar Widget&errmsg=0",
					$template);
				redirect($url);	
				exit;
			}
	...
	if ($row = mysql_fetch_assoc($result)) {
		...
		if (!isset($title) || $title == '') {
			$url = sprintf("error.php?%s&errtitle=Erro ao editar Widget&errmsg=Digite um nome no campo Nome.",
				$template);
			redirect($url);	
			exit;
		}
