...
if (isset($_REQUEST['create'])) {
	unset($currentwidget);
	query("update opensocial set widget_id = NULL where id = '" . $user . "'");
	...
	if (isset($code) && isset($title) && $code != '' && $title != '') {
		...
		if (mysql_fetch_assoc($result)) {
			$url = sprintf("errormsg.php?%s&errtitle=Error al crear Widget&errmsg=0",
				$template);
			redirect($url);
			exit;
		}
			...
			if ($row = mysql_fetch_assoc($result)) {
				...
				query("update opensocial set widget_id = " . $currentwidget . " where id = '" . $user . "'");

				//redirect('index.php?update');
				//exit;
			}
		}
	}
	else {
		$url = sprintf("errormsg.php?%s&errtitle=Error al crear Widget&errmsg=1",
			$template);
		redirect($url);
		exit;
	}
	...
	if ($row = mysql_fetch_assoc($result)) {
		...
		if($cube_id == $currentcube) {
			...
			if(!isset($title) || $title == '') {
				$url = sprintf("error.php?%s&errtitle=Error al copiar Widget&errmsg=Por favor, entre un nombre en el campo de Nombre.",
					$template);
				redirect($url);	
				exit;
			}
			...
			if (mysql_fetch_assoc($result)) {
				$url = sprintf("errormsg.php?%s&errtitle=Error al copiar Widget&errmsg=0",
					$template);
				redirect($url);	
				exit;
			}
	...
	if ($row = mysql_fetch_assoc($result)) {
		...
		if (!isset($title) || $title == '') {
			$url = sprintf("error.php?%s&errtitle=Error al editar Widget&errmsg=Por favor, entre un nombre en el campo de Nombre.",
				$template);
			redirect($url);	
			exit;
		}
