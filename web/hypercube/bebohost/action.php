...
if (isset($_REQUEST['create'])) {
	unset($currentwidget);
	query("update bebo set widget_id = NULL where fb_sig_user = " . $user);
	...
	if (isset($code) && isset($title) && $code != '' && $title != '') {
		...
		if (mysql_fetch_assoc($result)) {
			redirect("error.php?errtitle=Widget creation error&errmsg=Please enter a unique name for this widget.");
			exit;
		}
		else {
		...
		redirect("error.php?errtitle=Widget creation error&errmsg=Please fill out both the Name and Code fields.");
		exit;
	}
}
	...
	if ($row = mysql_fetch_assoc($result)) {
		...
		if($cube_id == $currentcube) {
			...
			if(!isset($title) || $title == '') {
				redirect("error.php?errtitle=Widget copy error&errmsg=Please enter a name into the Name field.");
				exit;
			}
			...
			if (mysql_fetch_assoc($result)) {
				redirect("error.php?errtitle=Widget copy error&errmsg=Please enter a unique name for this widget.");
				exit;
			}
			else {
	...
	if ($row = mysql_fetch_assoc($result)) {
		...
		if (!isset($title) || $title == '') {
			redirect("error.php?errtitle=Widget edit error&errmsg=Please enter a name into the Name field.");
			exit;
		}
