...
- (void)mouseEntered:(NSEvent*)theEvent
{
	...
	int level = [[self window] level];
	if(level == NSFloatingWindowLevel || level == NSPopUpMenuWindowLevel) {
		GetFrontProcess(&lastFrontProcess);
		
		ProcessSerialNumber currentProcess;
		GetCurrentProcess(&currentProcess);
		
		if(IsProcessVisible(&currentProcess)) {
			Boolean result;
...
- (void)drawRect:(NSRect)aRect
{
	...
	[bgPath appendBezierPathWithArcFromPoint:frame.origin 
									 toPoint:NSMakePoint(midX, minY) 
									  radius:radius];
	[bgPath closePath];
	[[NSColor colorWithDeviceWhite:0.1 alpha:.90] set];
	[bgPath fill];
}
