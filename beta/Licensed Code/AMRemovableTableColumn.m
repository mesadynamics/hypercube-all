...
- (BOOL)isHidden
{
	if (AMRemovableTableColumn_frameworkDoesSupportHiddenColumns) {
		return [super isHidden]; // cast to id to avoid compiler warning
	} else {
		return (![self tableView] || ([self tableView] && (![[[self tableView] tableColumns] containsObject:self])));
	}
}
...
{
	if (AMRemovableTableColumn_frameworkDoesSupportHiddenColumns) {
		[super setHidden:flag]; // cast to id to avoid compiler warning
	} else {
		if (flag) {
